...
DOM.controls.appendChild(btnBack);
...
function initEventListeners() {
    ...
    window.addEventListener("click", (event) => {
        if (event.target === DOM.modal) DOM.modal.style.display = "none";
    });
    ...
    DOM.btnMetadata.addEventListener("click", () => DOM.metadataSidebar.classList.toggle("open"));
    DOM.btnCloseSidebar.addEventListener("click", () => DOM.metadataSidebar.classList.remove("open"));

    DOM.viewModeRadios.forEach(radio => {
        radio.addEventListener("change", (e) => updateView(e.target.value));
    });

    DOM.fileInputA.addEventListener("change", (e) => handleFile(e, (data) => {
        dataA = data;
        handleDataUpdate();
    }));

    DOM.fileInputB.addEventListener("change", (e) => handleFile(e, (data) => {
        dataB = data;
        handleDataUpdate();
    }));

    DOM.btnResetLayout.addEventListener("click", () => handleDataUpdate());
    DOM.btnZoomIn.addEventListener("click", () => setZoom(zoomLevel * 1.1));
    DOM.btnZoomOut.addEventListener("click", () => setZoom(zoomLevel / 1.1));
...
function updateView(mode) {
    ...
    DOM.btnMetadata.disabled = !hasMetadata;
    if (!hasMetadata) {
        DOM.metadataSidebar.classList.remove("open");
    }
}
...
function handleDataUpdate() {
    ...
    currentDiffData = comparePatches(dataA, dataB);
    DOM.viewToggles.style.display = "block";
    DOM.btnMetadata.style.display = "inline-block";
...
async function checkLocalServer() {
    try {
        ...
        if (response.ok) {
            ...
            DOM.fileInputsDiv.style.display = "none";
...
function comparePatches(dataA, dataB) {
    ...
    for (const [id, boxA] of dictA.entries()) {
        ...
        if (!boxB) {
            ...
            if (isContentModified) diffState = "modified";
            else if (isPositionModified) diffState = "moved";

            diffBoxes.push({...boxB, diffState, patcherA: boxA.patcher, patcherB: boxB.patcher, oldText: textA, attrDiffs});
        }
    }
...
function renderSingleMetadata(metadata) {
    DOM.metadataContent.innerHTML = "";
    for (const key in metadata) {
        ...
        const valDiv = document.createElement("pre");
        valDiv.className = "meta-new";
        valDiv.textContent = JSON.stringify(metadata[key], null, 2);
        div.appendChild(keyDiv);
...
function showDetails(box) {
    ...
    if (box.diffState === "modified" && box.attrDiffs?.length > 0) {
        box.attrDiffs.forEach(diff => {
            if (diff.key === "saved_attribute_attributes") {
                ...
                allAttrKeys.forEach(attrKey => {
                    ...
                    if (oldVal !== newVal) {
                        createAttrChangeElement(DOM.modalContent, `saved_attribute_attributes -> ${attrKey}`, oldVal, newVal);
                        hasContent = true;
                    }
                });
            } else {
                createAttrChangeElement(DOM.modalContent, diff.key, JSON.stringify(diff.old), JSON.stringify(diff.new));
                hasContent = true;
            }
        });
    }
    if (hasContent) DOM.modal.style.display = "block";
}
...
function applySpecialNaming(box) {
    ...
    if (box.maxclass === "bpatcher") suffix = box.name;
    return [prefix, suffix];
}

function getBoxDisplayName(box) {
    const attributes = box.saved_attribute_attributes?.valueof || {};
    const prettyName = attributes.parameter_longname || attributes.parameter_shortname;
    const basicName = box.text || box.maxclass;
    let displayName = prettyName || basicName;
    const subText = (prettyName && prettyName !== basicName) ? basicName : null;
...
function render(boxes, lines, isDiff) {
    ...
    DOM.container.style.width = `${maxX + 200}px`;
    DOM.container.style.height = `${maxY + 200}px`;
...
function renderBoxContent(el, b, isDiff) {
    ...
    if (isDiff && b.diffState === "modified" && b.oldText && b.oldText !== (b.text || b.maxclass) && !displayInfo.sub) {
        ...
        newSpan.textContent = displayInfo.main;
        ...
        span.textContent = displayInfo.main;
...
function createConnectionPath(src, dst, line) {
    ...
    const startY = sy + sh;
    const endX = dx + (dw / (dstInlets + 1)) * (line.destination[1] + 1);
    const endY = dy;
...
function setZoom(newLevel, pivot) {
    ...
    const px = pivot ? pivot.x - rect.left : DOM.wrapper.clientWidth / 2;
    const py = pivot ? pivot.y - rect.top : DOM.wrapper.clientHeight / 2;

    const ratio = newLevel / oldLevel;
    const newScrollLeft = (px + DOM.wrapper.scrollLeft) * ratio - px;
    const newScrollTop = (py + DOM.wrapper.scrollTop) * ratio - py;

    zoomLevel = newLevel;
    DOM.container.style.transform = `scale(${zoomLevel})`;
    DOM.btnZoomReset.textContent = `${Math.round(zoomLevel * 100)}%`;
    DOM.wrapper.scrollLeft = newScrollLeft;
    DOM.wrapper.scrollTop = newScrollTop;
}
...
function updateConnectedLines(movedBox, lines, boxDict) {
    lines.forEach(l => {
        if (l.source[0] === movedBox.id || l.destination[0] === movedBox.id) {
            ...
            const path = DOM.svgLayer.querySelector(selector);
            if (path) {
                const src = boxDict.get(l.source[0]);
                const dst = boxDict.get(l.destination[0]);
